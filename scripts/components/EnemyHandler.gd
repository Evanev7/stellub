...
var maximum_damage_numbers: int = 100
...
func spawn_damage_number(damage_value: float, position: Vector2, damage_numbers: Node2D):
	var damage_number = get_damage_number()
	if not damage_number:
		return
	var val = damage_value
	damage_number_spawner.add_child(damage_number, true)
	damage_number.set_values_and_animate(val, position, damage_number_spawner.get_child_count() * 5, 100 + 5 * damage_number_spawner.get_child_count())
